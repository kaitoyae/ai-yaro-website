...
- `src/components/`: ヘッダー、ローダー、WebGL 背景、共通 UI
...
3. `url` には公開して問題ない `https://` の GitHub、デモ、ドキュメントなどの URL を入れます。
...
- 外部リンクは `https://` のみ許可します。`http:`, `javascript:`, `data:` などの URL は掲載しないでください。
