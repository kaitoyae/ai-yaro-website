...
4. `npm run build` で壊れていないことを確認してから Pull Request を作成します。
...
- 依存関係を追加・更新した場合は、`npm run build` を通し、必要に応じて `npm audit` の結果を確認してください。
...
3. データ追加だけの場合も、公開して問題ない情報だけを含めてください。
4. レビューで指摘されたセキュリティ・プライバシー項目は優先して対応してください。
